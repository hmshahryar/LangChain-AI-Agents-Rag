...
class Review(BaseModel): # annotation to properly explain the things correctly 
    ...
    #  optional[int ] is a optopnal other wose default is should be given 
    # list[str] list of strings 
    key_themes:list[str] = Field(description="Write down al the key themes to discuss in reviews")
    summary:str = Field(description="Aberief summary for the reviews")
    sentiments: Literal["pos", "neg"] = Field(description="return the scentiment of the revies")
    pros:Optional[list[str]] = Field(default=None , description="write drown all the pros in side the list")
    cons:Optional[list[str]] = Field(default=None , description="write doen all the cons in the list")
    name:Optional[str] = Field(description="write down the names " , default=None) 
